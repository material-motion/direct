...
export class DirectSpec extends LitElement {
  static styles = css`
    .tab-container {
  ...
  render() {
    ...
    const specs = current?.multispec || current ? [current] : [];
    return html`<spec-header
      ...
      >
        <header>
          ...
          >
            ${specs.map(
              spec => html`<mwc-tab label="${spec?.title || ''}"> </mwc-tab>`
            )}
          </mwc-tab-bar>
