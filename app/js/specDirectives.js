    ...
    link: function ($scope, element, attrs) {
      var updateSize = function () {
        ...
        if (itemCanvasWrap.width() > 200) {
          ...
          var canvasMargin = itemCanvasWrap.outerWidth(true) -
                             itemCanvasWrap.outerWidth();
          var specItem = element.closest('.spec-item');
          if (canvasMargin + element.outerWidth() > specItem.width()) {
            var offset = specItem.width() - (canvasMargin + element.outerWidth());
...
app.directive('scrubbable', function () {
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      var gridContainment = $(element[0]).parents('.spec-content')
      ...
      $(gridContainment)
        .one('mouseover', '.spec-grid-scrubber', function() {
          var scrubber = $(this);
          scrubber.draggable({
            axis: 'x',
            cursorAt: { left: 0 },
      ...
      var checkDurationMatch = function() {
        if (video.duration && scope.spec.duration) {
          var warn = Math.abs(video.duration * 1000 - scope.spec.duration) > 100;
