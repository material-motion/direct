...
app.controller('specTabCtrl', function ($scope, $location, $filter, $sce, Spec, storageService) {
  // Initial Config
  $scope.canEdit = false;
  $scope.editing = false;
  $scope.overlay = false;
  ...
  $scope.getVideoSrc = function (src) {
    ...
    if (src && src.search(ID_REGEX) != -1) {
      ...
      // Is a drive ID
      return $sce.trustAsResourceUrl('https://docs.google.com/uc?authuser=0&id=' + videoId + '&export=download');
  ...
  var recalculate = function () {
    if ($scope.spec) {
      ...
      $scope.spec.divisions.minor = msPerFrame * $scope.spec.divisions.minorFrames;
      $scope.spec.divisions.major =  msPerFrame * $scope.spec.divisions.majorFrames;
  ...
  $scope.$watchGroup([
    ...
    'spec.divisions.majorFrames'], function () {
      recalculate();
  });
  ...
  $scope.$watchGroup(['spec.duration', 'spec.divisions'], function (newValues, oldValues, scope) {
    $scope.refreshCanvas();
  });
