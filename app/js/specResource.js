...
app.provider('Spec', function () {
  this.$get = ['$resource', 'envService', 'localStorageService', '$timeout',
    function ($resource, envService, localStorageService, $timeout) {

    if (envService.is('production')) {
      var Spec = $resource('/api/spec/:id', {}, {
        update: {
          method: 'PUT'
        }
      });
      ...
      var Spec = {
        query: function(params, callback) {
          ...
          $timeout(function() { // simulate async so the callback works
            var specs = [];
        ...
        save: function(value, callback) {
          ...
          $timeout(function() { // simulate async so the callback works
            ...
            while (localStorageService.get(id)) id = Math.round(Math.random() * 100000000);
