...
app.service('storageService', function storageService($timeout, $location, Spec) {
  ...
  var autosave = function(scope) {
    ...
    scope.$watch('data.id', function(newVal, oldVal, scope) {
  ...
  var save = (function (scope) {
    // Update Spec
    Spec.update({ id: scope.data.id }, sanitizedData(scope.data), function (resp) {
      console.log(resp);
      saveFinished(scope);
    }, function(error) {
      saveFinished(scope);
  ...
  var create = function(scope) {
    // Create Spec
    Spec.save(sanitizedData(scope.data), function (resp) {
      console.log(resp);
